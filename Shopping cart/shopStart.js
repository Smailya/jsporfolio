...
function sub_selection(x) {
   ...
   if (quantities[x]>0){ quantities[x] = quantities[x] - 1;
    totals[x] = prices[x] * quantities[x];
    totalOrderAmt -= prices[x];
...
function display_all() {
    ...
    myTable += "<th style='width: 100px; color: red; text-align: right;'>Sub</th>";
    ...
    for (i = 0; i < items.length; i++) {
        ...
        myTable += "<td><button onclick='sub_selection(" + i + ")'>Sub</button></td>";
    }
...
document.getElementById("sumtotal").addEventListener("click",displayTotal)

//var tot;

//var tot = document.getElementById("subtotal").onclick = disp ;
